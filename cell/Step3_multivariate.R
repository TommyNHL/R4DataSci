...
log2.cpm.filtered.norm.df <- read_csv("../myDGEListSRR275967log2_filtered_normalized.csv")
...
mydata.df <- log2.cpm.filtered.norm.df %>% 
    mutate(healthy.AVG = (`LX-2_control1` + `LX-2_control2` + `LX-2_control3`)/3, 
           disease.AVG = (`LX-2_treat1` + `LX-2_treat2` + `LX-2_treat3`)/3, 
