...
load(file = "../myDGEListSRR275967FilterNorm")
...
ebFit <- eBayes(fits)
write.fit(ebFit, file="../lmfitSRR275967_results.txt")

# ==============================================================================

# TopTable to view DEGs -----
#myTopHits <- topTable(ebFit, adjust ="BH", coef=1, number=10, sort.by="logFC")
myTopHits <- topTable(ebFit, adjust ="BH", coef=1, number=16018, sort.by="logFC")
...
write.csv(temp, "../myDGEListSRR275967log2_filtered_normalized_stat.csv")
...
vplot <- ggplot(myTopHits.df) + 
    ...
    annotate("rect", xmin = 1, xmax = 8, ymin = -log10(0.05), ymax = 15, alpha=.2, fill="#BE684D") + 
    annotate("rect", xmin = -1, xmax = -8, ymin = -log10(0.05), ymax = 15, alpha=.2, fill="#2C467A") + 
    labs(title="RNA-Seq: TGFβ-Treated LX-2 Cells & Control Groups") +
    theme_bw()
...
EnhancedVolcano(myTopHits.df, 
                ...
                #title = "", 
                subtitle = "RNA-Seq: TGFβ-Treated LX-2 Cells & Control Groups", 
...
write_csv(diffGenes.df, "../DiffGenesSRR275967.csv")
